...
void net::mrmt::setReuseAddr(int socket_fd, bool on) {
    int optval = on ? 1 : 0;
...
void net::mrmt::setReusePort(int socket_fd, bool on) {
    int optval = on ? 1 : 0;
...
void net::mrmt::setNoBlock(int socket_fd) {
    // non-block
...
void net::mrmt::setCloseOnExec(int socket_fd) {
    // close-on-exec
